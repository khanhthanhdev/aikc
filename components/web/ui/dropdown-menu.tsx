...
>(({ className, sideOffset = 4, ...props }, ref) => (
  <DropdownMenuPrimitive.Portal>
    <DropdownMenuPrimitive.Content
      className={cx(
        "!duration-150 z-50 flex min-w-[8rem] flex-col overflow-hidden rounded-md border bg-background/25 p-1 backdrop-blur-xl",
...
>(({ className, children, checked, ...props }, ref) => (
  <DropdownMenuPrimitive.CheckboxItem
    checked={checked}
    className={cx(
      "relative flex select-none items-center rounded-sm py-1.5 pr-2 pl-8 text-sm outline-none focus:bg-foreground/10 focus:text-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
      className
    )}
    ref={ref}
    {...props}
...
>(({ className, ...props }, ref) => (
  <DropdownMenuPrimitive.Separator
    className={cx("-mx-1 my-1 h-px bg-foreground/15", className)}
    ref={ref}
    {...props}
  />
));
