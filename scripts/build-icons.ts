...
async function main() {
  ...
  const symbols = await Promise.all(
    files.map(async (file) =>
      svgToSymbol(path.join(inputDir, file), iconName(file))
    )
  );
...
`;
  ...
  if (spriteChanged || typesChanged) {
    console.log(
      `Generated ${files.length} icons -> ${path.relative(cwd, spriteFilepath)}`
    );
  } else {
...
async function svgToSymbol(filepath: string, id: string) {
  ...
  const attributes = Array.from(
    openTagMatch[0].matchAll(/([\w:-]+)="([^"]*)"/g)
  ).filter(
    ([, name]) =>
      !["width", "height", "xmlns", "xmlns:xlink", "version"].includes(name)
  );
